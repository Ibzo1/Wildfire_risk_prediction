...
def main(parameter_set_key: str = 'default',
    ...
    train_indices, test_indices = get_indices(rawdata_df, train_range,
                                              test_range)  # set for fire season only unless changed
    ...
    reshaped_data, reshaped_labels, reshaped_masks = reshape_data(rawdata_df, features, target_column, device_set,
                                                                  include_masks=include_masks, tolerance=mask_size)
    ...
    logging.info(
        f"Data split successfully, train_set size - {len(X_train)}, val_set size - {len(X_val)}, test_set size - {len(X_test)}")
    ...
    if generate_predictions:
        ...
        with torch.no_grad():
            ...
            for i in X_test:
                ...
                i_df = rawdata_df[rawdata_df['date'] == dates[i+prediction_day]].loc[:, ['date', 'latitude', 'longitude', target_column]]
                # add the risk_score to df
                i_df['risk_score'] = outputs.detach().cpu().numpy().reshape(1258)
